...
> & {
  ...
  return (
    <PopoverPrimitive
      ...
      triggerRef={anchor}
      className={cn("relative isolate z-50 max-h-72 w-(--trigger-width) min-w-32 origin-(--trigger-anchor-point) overflow-hidden rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 data-entering:animate-in data-entering:fade-in-0 data-entering:zoom-in-95 data-exiting:animate-out data-exiting:fade-out-0 data-exiting:zoom-out-95 data-[placement=bottom]:slide-in-from-top-2 data-[placement=left]:slide-in-from-right-2 data-[placement=right]:slide-in-from-left-2 data-[placement=top]:slide-in-from-bottom-2 **:data-[slot$=-item]:data-focused:bg-foreground/10 *:data-[slot=input-group]:m-1 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-7 *:data-[slot=input-group]:border-none *:data-[slot=input-group]:bg-card *:data-[slot=input-group]:shadow-none dark:bg-popover", className )}
      {...props}
    />
  )
}
...
function ComboboxChips({ children, className, ...props }: React.ComponentProps<typeof Group>) {
  return (
    <Group
...
function ComboboxChipsInput({ className, ...props }: InputProps) {
  ...
  return (
    <InputPrimitive
      ...
      className={cn(
        "min-w-16 flex-1 border-0 bg-transparent shadow-none outline-none ring-0 focus:ring-0! focus-visible:ring-0!",
        className,
      )}
      onKeyDown={(e) => {
        if (

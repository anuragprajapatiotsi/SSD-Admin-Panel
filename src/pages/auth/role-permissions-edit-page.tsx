...
function CreateRoleForm({ roleCode, scope, disabled, errors, onCancel, onCode, onScope, onSubmit }: { roleCode: string; scope: string; disabled: boolean; errors: { roleCode?: string; roleScope?: string }; onCancel: () => void; onCode: (value: string) => void; onScope: (value: string) => void; onSubmit: () => void }) {
  ...
  return <form className="mx-auto w-full max-w-3xl" noValidate onSubmit={(event) => { event.preventDefault(); onSubmit(); }}><Card><CardHeader><CardTitle>{t("pages.roleManagement.editor.details")}</CardTitle><CardDescription>{t("pages.rolePermissionWorkspace.createDetailsDescription")}</CardDescription></CardHeader><CardContent><FieldGroup><Field data-invalid={Boolean(errors.roleCode)}><FieldLabel htmlFor="role-code">{t("pages.roleManagement.editor.code")} <span className="text-destructive" aria-hidden="true">*</span></FieldLabel><Input className="w-full max-w-80" id="role-code" autoFocus maxLength={100} value={roleCode} disabled={disabled} aria-invalid={Boolean(errors.roleCode)} aria-required="true" onChange={(event) => onCode(event.target.value)} /><FieldDescription>{t("pages.roleManagement.editor.codeHelp")}</FieldDescription><FieldError>{errors.roleCode}</FieldError></Field><Field data-invalid={Boolean(errors.roleScope)}><FieldLabel>{t("pages.roleManagement.columns.scope")} <span className="text-destructive" aria-hidden="true">*</span></FieldLabel><ScopeSelect className="w-full max-w-80" value={scope} disabled={disabled} invalid={Boolean(errors.roleScope)} onChange={onScope} /><FieldDescription>{t("pages.rolePermissionWorkspace.scopeHelp")}</FieldDescription><FieldError>{errors.roleScope}</FieldError></Field></FieldGroup></CardContent><CardFooter className="flex-col-reverse gap-2 border-t sm:flex-row sm:justify-end"><Button className="w-full sm:w-auto" type="button" variant="outline" isDisabled={disabled} onPress={onCancel}>{t("pages.rolePermissionWorkspace.cancel")}</Button><Button className="w-full sm:w-auto" type="submit" isDisabled={disabled || !canContinue}>{t("pages.rolePermissionWorkspace.nextPermissions")}</Button></CardFooter></Card></form>;
}
function ScopeSelect({ className, value, disabled, invalid = false, onChange }: { className?: string; value: string; disabled: boolean; invalid?: boolean; onChange: (value: string) => void }) { const { t } = useTranslation("common"); return <Select aria-label={t("pages.roleManagement.columns.scope")} selectedKey={value} isDisabled={disabled} onSelectionChange={(key) => onChange(String(key))}><SelectTrigger className={cn("w-full max-w-40", className)} aria-invalid={invalid} aria-required="true"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem id="UNIT">{t("pages.roleManagement.scopes.unit")}</SelectItem><SelectItem id="GLOBAL">{t("pages.roleManagement.scopes.global")}</SelectItem></SelectGroup></SelectContent></Select>; }
